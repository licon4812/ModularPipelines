...
[<ModuleCategory("test")>]
[<DependsOn(typeof<CompileModule>, Optional = true)>]
...
type CategoryFilterDependencyTests() =
    ...
    member _.Optional_Dependency_Works_When_Filtered_By_Category() = async {
        ...
        do! check(Assert.That(result.ValueOrDefault).IsEqualTo("test-compile-skipped"))
    }
    ...
    member _.Both_Categories_Run_Successfully() = async {
        ...
        let! result = testModule.CompletionSource.Task |> Async.AwaitTask
        do! check(Assert.That(result.ValueOrDefault).IsEqualTo("test-with-compiled"))
    }
