...
type JsonSerializationTests() =
    ...
    member _.Test1() = async {
        ...
        do! check(Assert.That(deserializedSummary.Start).IsEqualTo(pipelineSummary.Start))
        do! check(Assert.That(deserializedSummary.End).IsEqualTo(pipelineSummary.End))
        do! check(Assert.That(deserializedSummary.TotalDuration).IsEqualTo(pipelineSummary.TotalDuration))
        do! check(Assert.That(deserializedSummary.Modules).HasCount().EqualTo(0))
