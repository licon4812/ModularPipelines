...
type HttpTests() =
    ...
    member private this.GetHttpWithFileLogger(file: string) =
        this.GetService<IHttp>(
            Action<IServiceCollection>(fun services ->
                services.AddLogging(fun builder ->
                    services.Configure<SpectreLoggerOptions>(fun options ->
                        options.MinimumLogLevel <- LogLevel.Information)
                    |> ignore

                    services.Configure<LoggerFilterOptions>(fun options ->
    ...
    member this.When_Log_Request_False_Then_Do_Not_Log_Request() = async {
        ...
        try
            let! result = this.GetHttpWithFileLogger(file) |> Async.AwaitTask
            let http = result.Item1
            let host = result.Item2
